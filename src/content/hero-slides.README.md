...
| `scrimStrength` | Contrast is short. `0`–`1`, multiplies the text bed only. `check:hero` prints the minimum value that passes. Below `1` lightens the bed for an already-dark image. |
...
needs `scrimStrength` above about `0.8`, it is the wrong photograph — the scrim
will be doing so much work that the picture is no longer doing any.
